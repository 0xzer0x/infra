...
with lib;
let
  cfg = config.features.cli.ansible;
in
{
  options.features.cli.ansible.enable = mkEnableOption "Enable Ansible CLI utilities";
  ...
  config = mkIf cfg.enable {
    ...
    home.packages =
      let
        ansible = (
          pkgs.ansible.overridePythonAttrs (finalAttrs: {
            dependencies = finalAttrs.dependencies ++ (with pkgs.python3Packages; [ boto3 ]);
          })
        );
      in
      [ ansible ];
    ...
    home.sessionVariables = {
      ...
      ANSIBLE_COLLECTIONS_PATH = "${pkgs.python3Packages.ansible}/lib/python${pkgs.python3.pythonVersion}/site-packages";
    };
  };
}
